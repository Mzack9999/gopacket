...
type SocketStatsV3 tpacketStatsV3
...
func (h *TPacket) setUpRing() (err error) {
	...
	case TPacketVersion1, TPacketVersion2:
		var tp tpacketReq
		tp.Block_size = uint32(h.opts.blockSize)
		tp.Block_nr = uint32(h.opts.numBlocks)
		tp.Frame_size = uint32(h.opts.frameSize)
		tp.Frame_nr = uint32(h.opts.framesPerBlock * h.opts.numBlocks)
	...
	case TPacketVersion3:
		...
		tp.Frame_size = uint32(h.opts.frameSize)
		tp.Frame_nr = uint32(h.opts.framesPerBlock * h.opts.numBlocks)
		tp.Retire_blk_tov = uint32(h.opts.blockTimeout / time.Millisecond)
		if err := setsockopt(h.fd, unix.SOL_PACKET, unix.PACKET_RX_RING, unsafe.Pointer(&tp), unsafe.Sizeof(tp)); err != nil {
...
func (h *TPacket) SocketStats() (SocketStats, SocketStatsV3, error) {
	...
	if h.tpVersion == TPacketVersion3 {
		...
		h.socketStatsV3.freezeQCnt += ssv3.freezeQCnt
